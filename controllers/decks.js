...
module.exports.postCardToDeck = async(req, res, next) => {
	...
	if(!card) {
		card =  new db.Card({name: req.body.name, colors: req.body.colors, type: req.body.type, image: req.body.image, multiverseid : req.body.multiverseid, description: req.body.description, supertypes : req.body.supertypes, types : req.body.types, rarity : req.body.rarity, manaCost : req.body.manaCost});
	...
	if(limit >= 60) {
		...
		let err = new expressError('You have already reached the deck card limit (60). Cannot add more cards!', 401);
		next(err);
		//res.end();
	}
	
	//POC For Limiting the number of cards to 4 each in each deck
	let cardNumber = deck.cards.filter(x => x.name === card.name && !x.supertypes.includes("Basic") && !x.types.includes("Land")).length;
...
const filterDeck = products => {
	...
	products.forEach(function (arrayItem) {
		...
		cards.push({
			...
			let count = cards[cards.findIndex(el => el.name === arrayItem.name)].count + 1;
			...
			cards[cards.findIndex(el => el.name === updatedCard.name)] = updatedCard;
		}
	})
	return cards;
}
